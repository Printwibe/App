  ...
  try {
    ...
    for (let i = 0; i < cart.items.length; i++) {
      ...
      if (
        ...
        item.customizationData.designLibrary &&
          ...
          try {
            ...
            if (viewDesign.preview) {
              try {
                ...
                const previewBlob = new Blob([previewByteArray], {
                  type: "image/png",
                });
            ...
            const designsCollection =
              db.collection<CustomDesign>("customDesigns");
        ...
        uploadedDesignsMap.set(i, {
          viewDesigns: uploadedDesigns,
          notes: item.customizationData.notes,
        });
      }
        ...
        for (const areaType of [
          ...
          try {
            ...
            const fileBlob = new Blob([byteArray], {
              type: tempDesign.fileType || "image/png",
            });

            // Upload to Vercel Blob with organized path
            const timestamp = Date.now();
            const sanitizedFilename =
              tempDesign.fileName?.replace(/[^a-zA-Z0-9.-]/g, "_") ||
    ...
    const order: Order = {
      ...
      items: cart.items.map((item, index) => {
        ...
        return {
          ...
          viewDesigns: uploadedDesigns?.viewDesigns
            ? uploadedDesigns.viewDesigns
            : undefined,
          // Legacy format: customDesigns (front/back/wraparound/preview)
          customDesigns:
            uploadedDesigns &&
            (uploadedDesigns.front ||
              uploadedDesigns.back ||
              uploadedDesigns.wraparound ||
              uploadedDesigns.preview)
              ? {
      ...
      razorpayPaymentId,
      manualPaymentDetails:
        paymentMethod === "manual_upi" || paymentMethod === "manual_qr"
          ? manualPaymentDetails
          : undefined,
    ...
    await Notifications.create({
      ...
      message: `Order ${orderId} has been placed${
        hasCustomizedItems ? " with custom designs" : ""
    ...
    const emailData = {
      ...
      shippingAddress,
      paymentMethod,
      customizedItems: hasCustomizedItems,
    };
