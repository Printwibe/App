    ...
    const enrichedItems = await Promise.all(
      cart.items.map(async (item) => {
        ...
        return {
          ...
          customDesignUrl: item.customDesignId
            ? `/uploads/designs/${item.customDesignId}`
            : undefined,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch product details to include in cart item
  ...
  try {
    ...
    if (quantity <= 0) {
      ...
      await db
        ...
        .updateOne(
          { userId: user._id },
          { $pull: { items: null as unknown as CartItem } }
        );
    } else {
      await db.collection<Cart>("carts").updateOne(
        { userId: user._id },
    ...
    if (itemIndex !== null) {
      ...
      if (cart) {
        const newItems = cart.items.filter(
          (_, i) => i !== Number.parseInt(itemIndex)
        );
        await cartsCollection.updateOne(
          { userId: user._id },
