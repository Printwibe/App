...
export const metadata: Metadata = {
  title: "Terms & Conditions | PrintWibe",
  description: "Read our terms and conditions for using PrintWibe services",
}

async function getTermsPage(): Promise<LegalPage | null> {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"
    const response = await fetch(`${baseUrl}/api/legal-pages?type=terms`, {
      cache: "no-store",
    })
            ...
            <CardHeader>
              ...
              {termsPage && (
                <p className="text-sm text-muted-foreground">
                  Last updated: {new Date(termsPage.updatedAt).toLocaleDateString()} | Version: {termsPage.version}
                </p>
              )}
            </CardHeader>
            <CardContent>
