  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data.settings?.shippingCost !== undefined) {
            setShippingCost(Number(data.settings.shippingCost) || 0);
          }
        }
      } catch (error) {
  ...
  const revalidatePromoCode = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setPromoMessage(
          `⚠ Promo code removed: ${data.error || "No longer valid for current cart value"}`,
        );
  ...
  const applyPromoCode = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (showMessage) {
          setPromoMessage(
            `✓ ${data.discount > 0 ? `${currencySymbol}${data.discount.toFixed(2)} discount applied!` : "Code applied"}`,
          );
        }
      } else {
        setDiscount(0);
