...
export function ProductDetails({ product }: ProductDetailsProps) {
  ...
  // Use first valid size/color; fallback to variant values so cart variant resolution still works
  const uniqueSizes = [
    ...new Set(product.variants.map((v) => v.size).filter((s) => !!s)),
  ];
  const uniqueColors = [
  ...
  const [selectedColor, setSelectedColor] = useState(
    uniqueColors[0] || product.variants[0]?.color || "",
  );
  ...
  // Get unique sizes and colors (skip empty values)
  ...
  const totalPrice =
    product.basePrice + (wantsCustomization ? product.customizationPrice : 0);
          ...
          <div className="flex items-baseline gap-3">
            ...
            {wantsCustomization && (
              <span className="text-sm text-muted-foreground">
                ...
                {product.customizationPrice.toFixed(2)} customization fee)
              </span>
            )}
          </div>

          {/* Size Selection (hide if none or only one) */}
          {sizes.length > 1 && (
