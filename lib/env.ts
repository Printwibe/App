...
function validateEnv(): Env {
  ...
  for (const insecure of insecureValues) {
    if (jwtSecret.toLowerCase().includes(insecure)) {
      throw new Error(`JWT_SECRET contains insecure default value. Please generate a secure random secret.`)
    }
    if (adminJwtSecret.toLowerCase().includes(insecure)) {
      throw new Error(`ADMIN_JWT_SECRET contains insecure default value. Please generate a secure random secret.`)
    }
  }
